...
3. Create **_HTTP request Sampler_** to send requests to the web server with values for `Protocol`, `Server nam`, `HTTP Request Method` and `Path`
   Under `Body Data`: use built-in JMeter random function to generate dynamic data while creating user
...
7. The next request **Get User Profile** requires an access token in the authorization. Use _HTTP Header Manager_ to add the token as header
   Bearer ${user_token}
...
9. Add **_Listeners_** to view the results of the test execution:
    ...
    - `View Results in Table` to view latency, response size in bytes
...
3. Download the file Demo-load-test-with-RESTAPI.jmx can be found in this project
4. Open the file in GUI mode and click `run` button
## Run test with CLI and generate a report at the end of the test
1. Download the file Demo-load-test-with-RESTAPI.jmx can be found in this project
2. Open a command prompt, go to `bin` folder
3. To generate csv report: enter commmand `jmeter -n -t "location of the test file" -l "location of results file"`
   Example: `jmeter -n -t "templates\Demo-load-test-with-RESTAPI.jmx" -l "report1.csv"`
4. To generate html report: enter commmand `jmeter -n -t "location of the test file" -l "location of results file"` -e -o "location of report folder"`
